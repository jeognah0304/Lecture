...
#데이터 집어넣을 공간
location <- "C:/Users/parkjeongah/Desktop/working/studying major/사회조사분석사례연구/수업내용/20180516/plot/"

setwd("C:/Users/parkjeongah/Desktop/working/studying major/사회조사분석사례연구/수업내용/20180516")
all.data <- read.csv("사회조사사례연구_강의자료_11주차_퇴원_노령2012_다빈도질환5.csv.rbbw5ny.partial",header=T)


dim(all.data)
summary(all.data$dv18) # dv18=내원일수

#상위5%값을 알려줌
qnt <-quantile(all.data$dv18, probs= 0.95)

#상위5%의 사람은 비정상->그래서 정상범위에있는 사람만 가지고 계산하기 위해 한 작업들.
data <- all.data[all.data$dv18 <= qnt,]

dim(data) #상위5%의 값이 떨어져 나감.
...
#이미지 저장하는 방법, 3가지 셋트로 해야 이미지가 저장이 된다
# 그림을 그리면 그에 따른 설명도 붙여야 함.
...
#알고싶은것:Main_65세 질환별 재원일수 분포라서 65-69를 제목으로 붙ㅇ
...
#노인별로 5세단위별재 ㅈ원일수가 차이가 나는궁 ㄱ금해서 분석해본거고
#5세단위별로 박스플랏 그렸고->질환별로 5개의 질환별로 연령별 차이가 있는지 한번 본것.
#진료비 진료별로 재원일수 분포를 한번 본 것.
